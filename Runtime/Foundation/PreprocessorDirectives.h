...
#define DETAIL__ENUM_FLAGS(T, PREFIX_)                                                                                 \
    ...
    PREFIX_ constexpr T operator|(const T left, const T right) {                                                       \
    ...
    PREFIX_ constexpr T operator~(const T flags) {                                                                     \
        using type = std::underlying_type<T>::type;                                                                    \
        return static_cast<T>(~static_cast<type>(flags));                                                              \
    }                                                                                                                  \
    PREFIX_ constexpr T &operator|=(T &left, const T right) {                                                          \
    ...
    PREFIX_ constexpr bool HasFlag(const T flags, const T flagToTest) {                                                \
        using type = std::underlying_type<T>::type;                                                                    \
    ...
    PREFIX_ constexpr bool HasAnyFlags(const T flags, const T flagsToTest) {                                           \
        using type = std::underlying_type<T>::type;                                                                    \
        return (static_cast<type>(flags) & static_cast<type>(flagsToTest)) != 0;                                       \
    }                                                                                                                  \
    [[nodiscard]]                                                                                                      \
    PREFIX_ constexpr bool HasAllFlags(const T flags, const T flagsToTest) {                                           \
        using type = std::underlying_type<T>::type;                                                                    \
        return (static_cast<type>(flags) & static_cast<type>(flagsToTest)) == static_cast<type>(flagsToTest);          \
    }                                                                                                                  \
    [[nodiscard]]                                                                                                      \
    PREFIX_ constexpr T SetFlags(const T flags, const T flagsToSet) {                                                  \
        return (flags | flagsToSet);                                                                                   \
    }                                                                                                                  \
    [[nodiscard]]                                                                                                      \
    PREFIX_ constexpr T ClearFlags(const T flags, const T flagsToClear) {                                              \
        return (flags & ~flagsToClear);                                                                                \
    }                                                                                                                  \
    [[nodiscard]]                                                                                                      \
    PREFIX_ constexpr T SetOrClearFlags(const T flags, const T flagsToSetOrClear, bool value) {                        \
