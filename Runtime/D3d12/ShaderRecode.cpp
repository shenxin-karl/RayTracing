...
LocalRootParameterData::LocalRootParameterData(const RootSignature *pRootSignature)
    : _pRootSignature(pRootSignature), _serializeSize(0) {
...
void LocalRootParameterData::InitLayout(const RootSignature *pRootSignature) {
    ...
    for (size_t i = 0; i < rootParameters.size(); ++i) {
        ...
        case D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS: {
            _serializeSize = AlignUp(_serializeSize, sizeof(DWParam)) +
                             sizeof(DWParam) * rootParameter.Constants.Num32BitValues;
        ...
        case D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE: {
            ...
            _serializeSize = AlignUp(_serializeSize, sizeof(D3D12_GPU_DESCRIPTOR_HANDLE)) + sizeof(D3D12_GPU_DESCRIPTOR_HANDLE);
            break;
        }
        case D3D12_ROOT_PARAMETER_TYPE_CBV:
        case D3D12_ROOT_PARAMETER_TYPE_SRV:
        case D3D12_ROOT_PARAMETER_TYPE_UAV:
            _serializeSize = AlignUp(_serializeSize, sizeof(D3D12_GPU_VIRTUAL_ADDRESS)) + sizeof(D3D12_GPU_VIRTUAL_ADDRESS);
            break;
        default:
            Assert(false && "Invalid RootParameter Type!");
            break;
        }
    }
}
...
void LocalRootParameterData::SetConstants(size_t rootIndex,
    ...
    const float *pFloat = static_cast<const float *>(pData);
